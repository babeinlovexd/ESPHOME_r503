...
class R503FingerprintComponent : public PollingComponent, public uart::UARTDevice {
 public:
  ...
  void write_notepad(uint8_t page, const std::string &text);
  void read_notepad(uint8_t page);
  void read_index_table(uint8_t page);
  void cancel();
  void soft_reset();
  void get_image_ex();
  void check_sensor();
 ...
 protected:
  ...
  CallbackManager<void(uint8_t, const std::string &)> notepad_read_callback_;
};

template<typename... Ts> class EnrollmentAction : public Action<Ts...>, public Parented<R503FingerprintComponent> {
 public:
...
class CancelEnrollmentAction : public Action<Ts...>, public Parented<R503FingerprintComponent> {
 public:
  void play(const Ts &...x) override { this->parent_->finish_enrollment(1); }
};

template<typename... Ts> class DeleteAction : public Action<Ts...>, public Parented<R503FingerprintComponent> {
 public:
  TEMPLATABLE_VALUE(uint16_t, finger_id)
...
template<typename... Ts> class DeleteAllAction : public Action<Ts...>, public Parented<R503FingerprintComponent> {
 public:
  void play(const Ts &...x) override { this->parent_->delete_all_fingerprints(); }
};

template<typename... Ts> class LEDControlAction : public Action<Ts...>, public Parented<R503FingerprintComponent> {
 public:
...
template<typename... Ts> class AuraLEDControlAction : public Action<Ts...>, public Parented<R503FingerprintComponent> {
 public:
...
template<typename... Ts> class SetSecurityLevelAction : public Action<Ts...>, public Parented<R503FingerprintComponent> {
 public:
  TEMPLATABLE_VALUE(uint8_t, level)

  void play(const Ts &...x) override {
    auto level = this->level_.value(x...);
    this->parent_->set_security_level(level);
  }
};

template<typename... Ts> class WriteNotepadAction : public Action<Ts...>, public Parented<R503FingerprintComponent> {
 public:
  TEMPLATABLE_VALUE(uint8_t, page)
  TEMPLATABLE_VALUE(std::string, text)

  void play(const Ts &...x) override {
    auto page = this->page_.value(x...);
    auto text = this->text_.value(x...);
    this->parent_->write_notepad(page, text);
  }
};

template<typename... Ts> class ReadNotepadAction : public Action<Ts...>, public Parented<R503FingerprintComponent> {
 public:
  TEMPLATABLE_VALUE(uint8_t, page)

  void play(const Ts &...x) override {
    auto page = this->page_.value(x...);
    this->parent_->read_notepad(page);
  }
};

template<typename... Ts> class ReadIndexTableAction : public Action<Ts...>, public Parented<R503FingerprintComponent> {
 public:
  TEMPLATABLE_VALUE(uint8_t, page)

  void play(const Ts &...x) override {
    auto page = this->page_.value(x...);
    this->parent_->read_index_table(page);
  }
};

template<typename... Ts> class CancelAction : public Action<Ts...>, public Parented<R503FingerprintComponent> {
 public:
  void play(const Ts &...x) override {
    this->parent_->cancel();
  }
...
template<typename... Ts> class SoftResetAction : public Action<Ts...>, public Parented<R503FingerprintComponent> {
 public:
  void play(const Ts &...x) override {
    this->parent_->soft_reset();
  }
};

template<typename... Ts> class GetImageExAction : public Action<Ts...>, public Parented<R503FingerprintComponent> {
 public:
  void play(const Ts &...x) override {
    this->parent_->get_image_ex();
  }
};

template<typename... Ts> class CheckSensorAction : public Action<Ts...>, public Parented<R503FingerprintComponent> {
 public:
  void play(const Ts &...x) override {
    this->parent_->check_sensor();
  }
};
