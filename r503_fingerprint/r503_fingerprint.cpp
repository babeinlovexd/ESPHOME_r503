...
namespace esphome::r503_fingerprint {

static const char *const TAG = "r503_fingerprint";

// Based on Adafruit's library: https://github.com/adafruit/Adafruit-Fingerprint-Sensor-Library

void R503FingerprintComponent::update() {
...
void R503FingerprintComponent::setup() {
...
void R503FingerprintComponent::enroll_fingerprint(uint16_t finger_id, uint8_t num_buffers) {
...
void R503FingerprintComponent::finish_enrollment(uint8_t result) {
  if (result == OK) {
...
void R503FingerprintComponent::scan_and_match_() {
  if (this->has_sensing_pin_) {
...
uint8_t R503FingerprintComponent::scan_image_(uint8_t buffer) {
  if (this->has_sensing_pin_) {
...
uint8_t R503FingerprintComponent::save_fingerprint_() {
...
bool R503FingerprintComponent::check_password_() {
...
bool R503FingerprintComponent::set_password_() {
...
bool R503FingerprintComponent::get_parameters_() {
...
void R503FingerprintComponent::get_fingerprint_count_() {
...
void R503FingerprintComponent::delete_fingerprint(uint16_t finger_id) {
...
void R503FingerprintComponent::set_security_level(uint8_t level) {
  ESP_LOGD(TAG, "Setting Security Level to %d", level);
  if (level < 1 || level > 5) {
    ESP_LOGE(TAG, "Invalid security level: %d. Must be between 1 and 5.", level);
    return;
  }
  this->data_ = {SET_SYS_PARAM, 0x05, level};
  if (this->send_command_() == OK) {
    ESP_LOGD(TAG, "Successfully set security level");
    this->get_parameters_();
  } else {
    ESP_LOGE(TAG, "Failed to set security level");
  }
}

void R503FingerprintComponent::write_notepad(uint8_t page, const std::string &text) {
  ESP_LOGD(TAG, "Writing to notepad page %d", page);
  if (page > 15) {
    ESP_LOGE(TAG, "Invalid notepad page: %d. Must be between 0 and 15.", page);
    return;
  }
  this->data_ = {WRITE_NOTEPAD, page};
  for (uint8_t i = 0; i < 32; i++) {
    if (i < text.length()) {
      this->data_.push_back(text[i]);
    } else {
      this->data_.push_back(0);
    }
  }
  if (this->send_command_() == OK) {
    ESP_LOGD(TAG, "Successfully wrote to notepad");
  } else {
    ESP_LOGE(TAG, "Failed to write to notepad");
  }
}

void R503FingerprintComponent::read_notepad(uint8_t page) {
  ESP_LOGD(TAG, "Reading from notepad page %d", page);
  if (page > 15) {
    ESP_LOGE(TAG, "Invalid notepad page: %d. Must be between 0 and 15.", page);
    return;
  }
  this->data_ = {READ_NOTEPAD, page};
  if (this->send_command_() == OK) {
    ESP_LOGD(TAG, "Successfully read from notepad");
    std::string text(this->data_.begin() + 1, this->data_.end());
    this->notepad_read_callback_.call(page, text);
  } else {
    ESP_LOGE(TAG, "Failed to read from notepad");
  }
}

void R503FingerprintComponent::read_index_table(uint8_t page) {
  ESP_LOGD(TAG, "Reading index table page %d", page);
  if (page > 3) {
    ESP_LOGE(TAG, "Invalid index table page: %d. Must be between 0 and 3.", page);
    return;
  }
  this->data_ = {READ_INDEX_TABLE, page};
  if (this->send_command_() == OK) {
    ESP_LOGD(TAG, "Successfully read index table");
    // Could add a callback for this if needed, but for now we'll just log it.
    std::string table = "";
    for (size_t i = 1; i < this->data_.size(); ++i) {
      char buf[4];
      sprintf(buf, "%02X ", this->data_[i]);
      table += buf;
    }
    ESP_LOGD(TAG, "Index table page %d: %s", page, table.c_str());
  } else {
    ESP_LOGE(TAG, "Failed to read index table");
  }
}

void R503FingerprintComponent::cancel() {
  ESP_LOGD(TAG, "Canceling current operation");
  this->data_ = {CANCEL};
  if (this->send_command_() == OK) {
    ESP_LOGD(TAG, "Successfully canceled operation");
  } else {
    ESP_LOGE(TAG, "Failed to cancel operation");
  }
}

void R503FingerprintComponent::soft_reset() {
  ESP_LOGD(TAG, "Performing soft reset");
  this->data_ = {SOFT_RESET};
  if (this->send_command_() == OK) {
    ESP_LOGD(TAG, "Successfully sent soft reset");
  } else {
    ESP_LOGE(TAG, "Failed to send soft reset");
  }
}

void R503FingerprintComponent::get_image_ex() {
  ESP_LOGD(TAG, "Getting image ex");
  this->data_ = {GET_IMAGE_EX};
  if (this->send_command_() == OK) {
    ESP_LOGD(TAG, "Successfully got image ex");
  } else {
    ESP_LOGE(TAG, "Failed to get image ex");
  }
}

void R503FingerprintComponent::check_sensor() {
  ESP_LOGD(TAG, "Checking sensor");
  this->data_ = {CHECK_SENSOR};
  if (this->send_command_() == OK) {
    ESP_LOGD(TAG, "Sensor is normal");
  } else {
    ESP_LOGE(TAG, "Sensor check failed");
  }
}

void R503FingerprintComponent::delete_all_fingerprints() {
...
void R503FingerprintComponent::led_control(bool state) {
...
void R503FingerprintComponent::aura_led_control(uint8_t state, uint8_t speed, uint8_t color, uint8_t count) {
...
uint8_t R503FingerprintComponent::transfer_(std::vector<uint8_t> &data_buffer) {
...
uint8_t R503FingerprintComponent::send_command_() {
  this->sensor_wakeup_();
  return this->transfer_(this->data_);
}

void R503FingerprintComponent::sensor_wakeup_() {
...
void R503FingerprintComponent::sensor_sleep_() {
...
void R503FingerprintComponent::dump_config() {
